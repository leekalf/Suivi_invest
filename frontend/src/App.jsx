...
// URL de l'API : Utilise la variable d'environnement en production, sinon localhost
...
function ProjectDetailPage() {
    ...
    const getIconForDetail = (label) => {
        ...
        if (l.includes('responsable')) return <PersonIcon fontSize="small" color="action" />;
        if (l.includes('contact') || l.includes('email')) return <EmailIcon fontSize="small" color="action" />;
        if (l.includes('capacité') || l.includes('production')) return <TrendingUpIcon fontSize="small" color="action" />;
        if (l.includes('agrément')) return <FactCheckIcon fontSize="small" color="action" />;
        if (l.includes('calendrier') || l.includes('date')) return <HistoryIcon fontSize="small" color="action" />;
        return <DescriptionIcon fontSize="small" color="action" />;
    };

    const renderDescriptionDetails = (description) => {
        if (!description) return <Typography variant="body2" sx={{ p: 2, fontStyle: 'italic' }}>Aucune information complémentaire disponible.</Typography>;
            ...
            if (parts.length > 1 && parts[0].trim()) {
                return (
                    <Grid item xs={12} sm={6} key={index}>
                        <Box sx={{ p: 1.5, borderBottom: '1px solid #f0f0f0', display: 'flex', alignItems: 'flex-start' }}>
                            <Box sx={{ mr: 1.5, mt: 0.5 }}>{getIconForDetail(parts[0])}</Box>
                            <Box>
                                <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 'bold', textTransform: 'uppercase', display: 'block' }}>{parts[0].trim()}</Typography>
                                <Typography variant="body2" sx={{ fontWeight: 500 }}>{parts[1].trim() || 'N/A'}</Typography>
                            </Box>
                        </Box>
                    </Grid>
                );
    ...
    return (
        <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            {/* EN-TÊTE PRINCIPAL */}
            <Paper elevation={0} sx={{ p: 4, mb: 3, borderRadius: 3, border: '1px solid #e0e0e0', background: 'linear-gradient(to right, #ffffff, #f8faff)' }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                    <Box>
                        <Stack direction="row" spacing={2} alignItems="center" sx={{ mb: 1 }}>
                            <BusinessIcon color="primary" sx={{ fontSize: 32 }} />
                            <Typography variant="h4" sx={{ fontWeight: 800, color: '#1a237e' }}>{project.name || "Projet sans nom"}</Typography>
                            <Chip 
                                label={project.status.toUpperCase()} 
                                color={project.status === 'en activités' ? 'success' : (project.status === 'suspendus' ? 'error' : 'primary')} 
                                size="small" 
                                sx={{ fontWeight: 'bold' }}
                            />
                        </Stack>
                        <Stack direction="row" spacing={3} sx={{ color: 'text.secondary' }}>
                            <Box sx={{ display: 'flex', alignItems: 'center' }}><PublicIcon sx={{ fontSize: 18, mr: 0.5 }} /> {project.investor?.country || 'Origine inconnue'}</Box>
                            <Box sx={{ display: 'flex', alignItems: 'center' }}><CategoryIcon sx={{ fontSize: 18, mr: 0.5 }} /> {project.investor?.sector || 'Secteur non défini'}</Box>
                            <Box sx={{ display: 'flex', alignItems: 'center' }}><LocationOnIcon sx={{ fontSize: 18, mr: 0.5 }} /> {project.land_area ? `${formatNumber(project.land_area)} m²` : 'Superficie N/A'}</Box>
                        </Stack>
                        <Typography variant="h6" sx={{ mt: 2, color: 'primary.main', fontWeight: 600 }}>Investisseur : {project.investor?.name}</Typography>
                    </Box>
                    <FormControl sx={{ minWidth: 180 }}>
                        <InputLabel>Année de reporting</InputLabel>
                        <Select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} size="small" label="Année de reporting">
                            {AVAILABLE_YEARS.map(year => (
                                <MenuItem key={year} value={year}>Année {year}</MenuItem>
                            ))}
                        </Select>
                ...
                <Grid container spacing={4}>
                    <Grid item xs={12}>
                        <Box sx={{ py: 2 }}>
                            <Typography variant="subtitle2" gutterBottom color="text.secondary" sx={{ textAlign: 'center', mb: 3, fontWeight: 'bold' }}>PROGRESSION DU DOSSIER</Typography>
                    ...
                    <Grid item xs={12} md={6}>
                        <Card variant="outlined" sx={{ borderRadius: 2 }}>
                            <CardHeader title="Performance Financière Mensuelle" titleTypographyProps={{ variant: 'subtitle1', fontWeight: 'bold' }} />
                            <CardContent>
                                <ResponsiveContainer width="100%" height={250}>
                                    <LineChart data={trends}>
                                        ...
                                        <YAxis hide />
                                        <Tooltip />
                                        <Line type="monotone" dataKey="inv" name="Invest. Réalisé" stroke="#388e3c" strokeWidth={3} dot={{ r: 4 }} />
                                        <Line type="monotone" dataKey="sales" name="Ventes" stroke="#1976d2" strokeWidth={3} dot={{ r: 4 }} />
                                    </LineChart>
                                </ResponsiveContainer>
                            </CardContent>
                        </Card>
                    ...
                    <Grid item xs={12} md={6}>
                        <Card variant="outlined" sx={{ borderRadius: 2 }}>
                            <CardHeader title="Suivi des Effectifs" titleTypographyProps={{ variant: 'subtitle1', fontWeight: 'bold' }} />
                            <CardContent>
                                <ResponsiveContainer width="100%" height={250}>
                                    <BarChart data={trends}>
                                        ...
                                        <Tooltip />
                                        <Legend />
                                        <Bar dataKey="men" name="Hommes" fill="#0277bd" radius={[4, 4, 0, 0]} />
                                        <Bar dataKey="women" name="Femmes" fill="#f06292" radius={[4, 4, 0, 0]} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </CardContent>
                        </Card>
                    ...
                    <Grid item xs={12} md={6}>
                        <Card sx={{ height: '100%', borderTop: '4px solid #f57c00' }}>
                            <CardHeader avatar={<TrendingUpIcon color="warning" />} title="Indicateurs d'Investissement" titleTypographyProps={{ fontWeight: 'bold' }} />
                            <CardContent>
                                <List dense>
                                    <ListItem divider><ListItemText primary={formatCurrency(project.investment_projected)} secondary="Budget Prévisionnel Total" primaryTypographyProps={{ fontWeight: 'bold', variant: 'h6' }} /></ListItem>
                                    <ListItem divider><ListItemText primary={formatCurrency(project.investment_realized)} secondary="Montant Réalisé à date" primaryTypographyProps={{ fontWeight: 'bold', variant: 'h6', color: 'success.main' }} /></ListItem>
                                    <ListItem><ListItemText primary={`${Math.round((project.investment_realized/project.investment_projected)*100)}%`} secondary="Taux de réalisation financière" primaryTypographyProps={{ fontWeight: 'bold' }} /></ListItem>
                                </List>
                            </CardContent>
                        </Card>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <Card sx={{ height: '100%', borderTop: '4px solid #1976d2' }}>
                            <CardHeader avatar={<PeopleAltIcon color="primary" />} title="Analyse du Capital Humain" titleTypographyProps={{ fontWeight: 'bold' }} />
                            <CardContent>
                                <Grid container spacing={2}>
                                    <Grid item xs={6}><Typography variant="caption" color="text.secondary">EM PLOIS EFFECTIFS</Typography><Typography variant="h5" fontWeight="bold">{formatNumber(project.employment?.jobs_effective)}</Typography></Grid>
                                    <Grid item xs={6}><Typography variant="caption" color="text.secondary">PRÉVISIONS RH</Typography><Typography variant="h5" fontWeight="bold" color="text.secondary">{formatNumber(project.employment?.jobs_forecast)}</Typography></Grid>
                                    <Grid item xs={12}><Divider sx={{ my: 1 }} /></Grid>
                                    <Grid item xs={3}><Typography variant="caption" sx={{ display: 'block' }}>Nationaux</Typography><Typography variant="body1" fontWeight="bold">{formatNumber(project.employment?.jobs_national)}</Typography></Grid>
                                    <Grid item xs={3}><Typography variant="caption" sx={{ display: 'block' }}>Expatriés</Typography><Typography variant="body1" fontWeight="bold">{formatNumber(project.employment?.jobs_expat)}</Typography></Grid>
                                    <Grid item xs={3}><Typography variant="caption" sx={{ display: 'block' }}>Hommes</Typography><Typography variant="body1" fontWeight="bold">{formatNumber(project.employment?.jobs_men)}</Typography></Grid>
                                    <Grid item xs={3}><Typography variant="caption" sx={{ display: 'block' }}>Femmes</Typography><Typography variant="body1" fontWeight="bold">{formatNumber(project.employment?.jobs_women)}</Typography></Grid>
                                </Grid>
                            </CardContent>
                        </Card>
                    </Grid>
                    ...
                    <Grid item xs={12}>
                        <Card variant="outlined" sx={{ bgcolor: '#fafafa' }}>
                            <CardHeader 
                                ...
                                title="Fiche Technique & Détails Opérationnels" 
...
function App() {
  ...
  const [selectedYear, setSelectedYear] = useState(CURRENT_YEAR);
  const [drawerOpen, setDrawerOpen] = useState(true); // État pour la sidebar
  ...
  const sidebarItems = [
    { text: 'Tableau de Bord', icon: <DashboardIcon />, path: '/', roles: ['ADMIN', 'MANAGER', 'AUDITEUR'] },
    { text: 'Courriers', icon: <Badge badgeContent={stats?.pending_requests} color="error"><EmailIcon /></Badge>, path: '/requests', roles: ['ADMIN', 'MANAGER'] },
    { text: 'Journal d\'Audit', icon: <HistoryIcon />, path: '/audit-logs', roles: ['ADMIN', 'MANAGER'] },
    { text: 'Utilisateurs', icon: <ManageAccountsIcon />, path: '/users', roles: ['ADMIN'] },
  ];

  return (
    <Box sx={{ display: 'flex', backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
      {/* SIDEBAR (DRAWER) PERSISTANTE */}
      {token && (
        <Drawer
          variant="permanent"
          sx={{
            ...
            '& .MuiDrawer-paper': {
              ...
              backgroundColor: '#1a237e', // Bleu Nuit Professionnel
              color: 'white',
              borderRight: 'none',
            },
          }}
        >
          <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: 64, justifyContent: 'center' }}>
            <Typography variant="h6" sx={{ fontWeight: 'bold', letterSpacing: 1, display: drawerOpen ? 'block' : 'none' }}> ACP INVEST </Typography>
            <Typography variant="h6" sx={{ fontWeight: 'bold', display: drawerOpen ? 'none' : 'block' }}> ACP </Typography>
            {drawerOpen && <Typography variant="caption" sx={{ opacity: 0.7 }}>Système de Gestion</Typography>}
          </Box>
            ...
            {sidebarItems.map((item) => (
              item.roles.includes(currentUser?.role) && (
                <ListItem key={item.text} disablePadding sx={{ mb: 1 }}>
                  <MuiTooltip title={!drawerOpen ? item.text : ""} placement="right">
                    <ListItemButton
                      component={Link}
                      to={item.path}
                      sx={{
                    ...
                    >
                      <ListItemIcon sx={{ color: 'white', minWidth: 0, mr: drawerOpen ? 2 : 'auto', justifyContent: 'center' }}>
                        {item.icon}
                      </ListItemIcon>
                      <ListItemText 
                        primary={item.text} 
          ...
          <Box sx={{ mt: 'auto', p: drawerOpen ? 2 : 1, textAlign: 'center' }}>
            <Divider sx={{ bgcolor: 'rgba(255,255,255,0.1)', mb: 2 }} />
            {currentUser && (
              <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, px: drawerOpen ? 1 : 0, justifyContent: 'center' }}>
                <Avatar sx={{ bgcolor: 'secondary.main', width: 32, height: 32, mr: drawerOpen ? 1.5 : 0 }}>
                  <PersonIcon />
                </Avatar>
                {drawerOpen && <Box sx={{ overflow: 'hidden', textAlign: 'left' }}>
                  <Typography variant="body2" noWrap sx={{ fontWeight: 'bold' }}>{currentUser.full_name}</Typography>
                  <Typography variant="caption" noWrap sx={{ opacity: 0.7, display: 'block' }}>{currentUser.role}</Typography>
                </Box>}
              </Box>
            )}
            <MuiTooltip title={!drawerOpen ? "Déconnexion" : ""} placement="right">
              <Button
                fullWidth
                startIcon={<LogoutIcon />}
                onClick={handleLogout}
                sx={{ color: '#ff8a80', justifyContent: drawerOpen ? 'flex-start' : 'center', px: drawerOpen ? 2 : 0, minWidth: 0 }}
              >
                {drawerOpen && "Déconnexion"}
              </Button>
            </MuiTooltip>
          </Box>
        </Drawer>
      ...
      <Box component="main" sx={{ 
        flexGrow: 1, 
        p: 0, 
        width: token ? `calc(100% - ${drawerOpen ? drawerWidth : collapsedDrawerWidth}px)` : '100%',
        transition: (theme) => theme.transitions.create('width', {
          easing: theme.transitions.easing.sharp,
          duration: theme.transitions.duration.enteringScreen,
        }),
      }}>
        <AppBar position="sticky" color="default" elevation={0} sx={{ borderBottom: '1px solid #e0e0e0', bgcolor: 'white' }}>
          <Toolbar sx={{ justifyContent: 'space-between' }}>
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              <IconButton onClick={toggleDrawer} sx={{ mr: 2, color: '#333' }}>
                <MenuIcon />
              </IconButton>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              {token ? (
                <>
                  {['ADMIN', 'MANAGER'].includes(currentUser?.role) && (
                    <Button variant="contained" startIcon={<AddIcon />} onClick={() => setOpenProjectForm(true)} sx={{ mr: 2 }} disableElevation>
                      Nouveau Dossier
                    </Button>
                  )}
                  <Button variant="outlined" startIcon={<DownloadIcon />} onClick={handleExportMenuClick} sx={{ mr: 1 }}>
                    Exporter
                  </Button>
                  <Menu anchorEl={anchorElExport} open={openExportMenu} onClose={handleExportMenuClose}>
                    <MenuItem onClick={handleExportProjectsExcel}>Projets Détaillés</MenuItem>
                    <MenuItem onClick={handleExportDashboardStatsExcel}>Statistiques Globales</MenuItem>
                    <MenuItem onClick={handleExportRequestsExcel}>Liste Courriers</MenuItem>
                  </Menu>
                </>
              ) : (
                <Button variant="contained" onClick={() => setOpenLogin(true)}>Connexion</Button>
              )}
            </Box>
          </Toolbar>
        </AppBar>
