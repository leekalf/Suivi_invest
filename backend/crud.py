...
def get_dashboard_stats(db: Session, year: Optional[int] = None):
    ...
    trend_query = db.query(
        # Utilise 'Mon YYYY' pour différencier les années si on regarde l'historique global
        func.to_char(models.MonthlyRecord.record_date, 'Mon YYYY').label('month'),
    ...
    # On groupe par le format 'Mon YYYY' et on trie par la date réelle pour garder l'ordre chronologique
    monthly_data = trend_query.group_by(
        func.to_char(models.MonthlyRecord.record_date, 'Mon YYYY'),
        func.date_trunc('month', models.MonthlyRecord.record_date)
    ).order_by(func.date_trunc('month', models.MonthlyRecord.record_date)).all()
